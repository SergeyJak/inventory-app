...
const imageDataPath = path.join(__dirname, 'yandex-plus-image.base64');
const imageDir = path.join(root, 'images', 'catalog', 'yandex-plus');
const imagePath = path.join(imageDir, '12-months.jpg');

// Materialize the exact Yandex Plus promo image from repository text data.
{
  const base64 = fs.readFileSync(imageDataPath, 'utf8').trim();
  fs.mkdirSync(imageDir, { recursive: true });
  fs.writeFileSync(imagePath, Buffer.from(base64, 'base64'));
  console.log('[yandex-plus] promo image restored');
}
