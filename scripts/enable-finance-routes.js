...
patch('server.js', source => {
  ...
  const routes = [
    ...
    ["['/account-status-fix.js', 'account-status-fix.js']", "  ['/account-status-fix.js', 'account-status-fix.js'],"],
  ];
...
patch('index.html', source => {
  ...
  if (!next.includes('/account-status-fix.js')) {
    const appAnchor = '<script src="app.js"></script>';
    if (!next.includes(appAnchor)) throw new Error('Account status fix script anchor not found');
    next = next.replace(appAnchor, appAnchor + '\n<script src="/account-status-fix.js?v=20260914-1"></script>');
  }
  return next;
});
